  ...
  return (
    <Layout>
      ...
      <div className="mt-10 mx-20">
        ...
        <p className="mt-5">こちらのフォームに必要情報のご入力をお願いいたします。担当より1営業日以内にメール、またはお電話にてご連絡させていただきます。（既にご利用中のお客様はサポートへのお問い合わせからご連絡ください）</p>
      </div>
      <form className="flex flex-col mt-10 mb-20 mx-20" onSubmit={handleSubmit}>
        <p className="font-semibold">お名前<span className="text-red-600">(必須)</span></p>
        <TextField required id="outlined-required" label="お名前" name="name" sx={{my: 1}}/>
        <p className="font-semibold">会社名<span className="text-red-600">(必須)</span></p>
        <TextField required id="outlined-required" label="会社名" name="company" sx={{my: 1}}/>
        <p className="font-semibold">メールアドレス<span className="text-red-600">(必須)</span></p>
        <TextField required id="outlined-required" label="メールアドレス" name="email" sx={{my: 1}}/>
        <p className="font-semibold">電話番号<span className="text-red-600">(必須)</span></p>
        ...
        <button className="bg-black hover:bg-slate-700 text-white font-medium mt-5 py-2 px-4 rounded w-max" type="submit" disabled={state.submitting}>送信する</button>
      </form>
    </Layout>
  )
}
